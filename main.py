...
def parse_args():
    ...
    parser.add_argument('--log_path', type=str, default='chat_logs.txt',
...
async def chat_client(host, port, log_path):
    ...
    async with aiofiles.open(log_path, mode='a', encoding='utf-8') as chat_logs:
        while True:
            logger.debug("Ожидание данных от сервера...")
            data = await reader.read(1024)
            if not data:
                logger.warning("Сервер закрыл соединение (получены пустые данные).")
                break
...
if __name__ == '__main__':
    args = parse_args()
    logger.info(f"Запуск клиента чата с аргументами: host={args.host}, port={args.port}, log_path={args.log_path}")
